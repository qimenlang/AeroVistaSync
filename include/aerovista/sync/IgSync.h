...
namespace aerovista::sync
{
    ...
    public:
        ...
        /// 注册某类 Host→IG 报文的到达回调：报文解包捕获时同步多播投递（状态同步设计初版.md §8.1）。
        ...
        /// 当前补偿后的模拟时间：内部 nowUs = `std::chrono::steady_clock::now()`。
        ...
        /// 帧头只填一次（去重），flushTcp 后重置（状态同步设计初版.md §8.1）。
    ...
    private:
        ...
        /// 解包一条 UDP 报文（主线程）：先 reset `IgCtrlCaptureProc`，再经 `_udpSession` 解包，
        /// 随后更新帧号/时间戳并回 SOF。眼点经 `_eyeProc` 回调投递，本函数不改眼点。
        ...
        // 数据面 I/O 线程（状态同步设计初版.md §5.1）：UDP recv（非阻塞）→ 入队 udpPayload；主线程 drainIncoming 解包。
        void startUdpThread();
        void stopUdpThread();
        void udpLoop();
        /// 创建 TCP 命令面 IG CCL 会话并注册基础设施 processor（Host→IG 一次性/配置/请求类，状态同步设计初版.md §8.1 按链路注册）。
        ...
        /// 创建 UDP 数据面 IG CCL 会话并注册通用捕获（IGCtrl / EntityPositionCtrl / 持续控制类，状态同步设计初版.md §8.1 按链路注册）。
        ...
        // 命令面 I/O 线程（状态同步设计初版.md §5.1）：TCP recv + 分帧 → 入队 tcpPayload；主线程 drainIncoming 解包。
        void startCommandThread();
        void stopCommandThread();
        void commandLoop();

        /// 注册单个通用捕获 processor（状态同步设计初版.md §8.1）：RegisterEventProcessor + 入 _captureProcs（供 addCallback 定位）。
        ...
        // TCP 侧 EntityPositionCtrlV4（命令实体 EntityID≠0）；与 UDP `_eyeProc` 双链路多播。
        ...
        // 数据面 I/O 线程（状态同步设计初版.md §5.1）：UDP recv → 入队；主线程 drainIncoming 解包。
