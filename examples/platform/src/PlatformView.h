...
private:
    ...
    CPlatformFrame* hostFrame() const;
...
inline CPlatformView* platformView(CWnd* from)
{
    if (from == nullptr)
        return nullptr;
    ...
    if (CPlatformView* view = DYNAMIC_DOWNCAST(CPlatformView, frame->GetActiveView()))
        return view;
    return DYNAMIC_DOWNCAST(CPlatformView, frame->GetDlgItem(AFX_IDW_PANE_FIRST));
}
