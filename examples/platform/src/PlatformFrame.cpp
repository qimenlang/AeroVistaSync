...
BOOL CPlatformFrame::PreCreateWindow(CREATESTRUCT& cs)
{
    ...
    cs.style |= CPlatformFrame::kFrameStyle;
    cs.style &= ~FWS_ADDTOTITLE;
    cs.dwExStyle &= ~WS_EX_CLIENTEDGE;
    cs.lpszName = _T("AeroVista platform");
    return TRUE;
}

bool CPlatformFrame::createDockPane(CDockablePane& pane, UINT id, LPCTSTR caption, DWORD barAlign,
...
bool CPlatformFrame::createPanes()
{
...
int CPlatformFrame::OnCreate(LPCREATESTRUCT createStruct)
{
    ...
    viewContext.m_pNewViewClass = RUNTIME_CLASS(CPlatformView);
    ...
    SetWindowText(_T("AeroVista platform"));
    return 0;
}

CPlatformView* CPlatformFrame::hostView()
{
    if (CPlatformView* view = DYNAMIC_DOWNCAST(CPlatformView, GetActiveView()))
        return view;
    return DYNAMIC_DOWNCAST(CPlatformView, GetDlgItem(AFX_IDW_PANE_FIRST));
}
...
BOOL CPlatformFrame::PreTranslateMessage(MSG* pMsg)
{
    ...
    if (CPlatformView* view = hostView())
    {
...
BOOL CPlatformFrame::LoadFrame(UINT resourceId, DWORD defaultStyle, CWnd* parentWnd, CCreateContext* context)
{
...
void CPlatformFrame::ActivateFrame(int nCmdShow)
{
    CFrameWndEx::ActivateFrame(nCmdShow);
    fitToFormView();
}

void CPlatformFrame::fitToFormView()
{
    RecalcLayout();
