...
{
    const std::optional<aerovista::sync::EntityAuthorityRow> row = _data.entityRow(_entityId);
    if (!row)
        return false;
...
{
    const auto row = _data.entityRow(_entityId);
    if (!row)
        return;
...
bool CEntityPropDlg::applyDirtyFields(std::string& error)
{
    ...
    if (ctrlDirty)
    {
        if (!_data.setEntityState(_entityId, state, &error))
            return false;
        if (!_data.setEntityAlpha(_entityId, alpha, &error))
            return false;
    }
    ...
    if (poseDirty)
    {
        ...
        pose.pitchDeg = pitch;
        pose.rollDeg = roll;
        if (!_data.setEntityPose(_entityId, pose, &error))
            return false;
    }
    ...
    if (ctrlDirty)
    {
        std::optional<CigiEntityCtrlV4> packet = _data.entityCtrlPacket(_entityId);
        if (!packet)
        {
    ...
    if (poseDirty)
    {
        std::optional<CigiEntityPositionCtrlV4> packet = _data.entityPositionPacket(_entityId);
        if (!packet)
        {
            error = "unknown entity id";
            return false;
